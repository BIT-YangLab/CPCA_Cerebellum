...
from utils.utils_cortex_subjs_srp import load_data_and_stack, write_to_cifti, \
write_to_gifti
...
def run_main(n_comps, n_sub, indi, path):
    ...
    '''
    analysis_str1 = f'/home/lvshuo/VDisk4/Lvshuo/2024_cerebellum_CPCA_rs-patterns_in_HCP/results_cortex/srp_cortex_{indi}'
    analysis_str2 = f'/home/lvshuo/VDisk4/Lvshuo/2024_cerebellum_CPCA_rs-patterns_in_HCP/results_cortex/crp_cortex_{indi}'
    phase, hdr, zero_mask = load_data_and_stack(n_sub, indi, path)
    
    tem = nib.load('/home/lvshuo/VDisk4/Lvshuo/visual_module/Yeo7_Yeo-Buckner-Choi-Raut.dlabel.nii').get_fdata()[:, :64984]
    
    srp_cere = np.zeros((n_comps, 64984))
    crp_cere = np.zeros((n_comps, 64984))
...
def write_results(srp_cere, crp_cere, n_comps, hdr, zero_mask, analysis_str1, analysis_str2):
    ...
    srp_all = np.zeros((n_comps, voxels))
    srp_all[:, :64984] = srp_cere
    write_to_cifti(srp_all, hdr, n_comps, f'{analysis_str1}')
    crp_all = np.zeros((n_comps, voxels))
    crp_all[:, :64984] = crp_cere
    write_to_cifti(crp_all, hdr, n_comps, f'{analysis_str2}')
