...
def visualize_cca_results(
    ...
    for i, p in enumerate(fdr_pvals):
        if p < 0.001:
            ax.text(i + 1, canon_corrs[i], "***", ha="center", fontsize=8)
        elif p < 0.01:
            ax.text(i + 1, canon_corrs[i], "**", ha="center", fontsize=8)
        elif p < 0.05:
            ax.text(i + 1, canon_corrs[i], "*", ha="center", fontsize=8)
...
def main(
    ...
    patterns, pattern_names : iterable
    ...
    # Cerebellum alone, cortex alone, whole-brain
    for stra in range(3):
        for comp in range(comps):
            for patt, patt_name in enumerate(['Complex', 'SRP']):
                save_prefix = save_path
                save_prefix = f'{save_prefix}{stra}_'
                
                # Step 1: Load network data
                data_file =  f"{base_path}{stra}_values_REST2_{patterns[patt]}_{comp}.txt"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        sys.argv += [
            "--path", "./source_files/MAD_selected_voxels/",
            "--behavior_csv", "./source_files/PCA_35_final4.csv",
            "--save_path", "./output/behavior_analysis/"
        ]
