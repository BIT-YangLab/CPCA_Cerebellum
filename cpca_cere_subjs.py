...
def run_main(n_comps, n_sub, global_signal, rotate, 
    ...
    '''
    analysis_str = f'results_cere/cpca_cere_{indi}'
...
def write_results(input_type, pca_output, rotate, shuffle_ts, simulate_var, comp_weights, 
    ...
    if pca_type == 'complex':
        ...
        comp_weights_amp = np.zeros((n_comps, voxels))
        comp_weights_amp[:, cere_s:cere_e] = comp_weights_amp_cere
        comp_weights_ang_cere = np.angle(comp_weights)
        comp_weights_ang = np.zeros((n_comps, voxels))
        comp_weights_ang[:, cere_s:cere_e] = comp_weights_ang_cere
        comp_weights_real_cere = np.real(comp_weights)
        comp_weights_real = np.zeros((n_comps, voxels))
        comp_weights_real[:, cere_s:cere_e] = comp_weights_real_cere
        comp_weights_imag_cere = np.imag(comp_weights)
        comp_weights_imag = np.zeros((n_comps, voxels))
        comp_weights_imag[:, cere_s:cere_e] = comp_weights_imag_cere
        if input_type == 'cifti':
