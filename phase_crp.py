...
from utils.utils_cere_subjs import load_data_and_stack, write_to_cifti, \
write_to_gifti
...
def run_main(n_comps, n_sub, indi, path):
    ...
    '''
    analysis_str1 = f'results_cere/srp_cere_{indi}'
    analysis_str2 = f'results_cere/crp_cere_{indi}'
    phase, hdr, zero_mask = load_data_and_stack(n_sub, indi, path)
    
    tem = nib.load('/home/lvshuo/VDisk4/Lvshuo/visual_module/Yeo7_Yeo-Buckner-Choi-Raut.dlabel.nii').get_fdata()[:, 65289:83142]
    
    srp_cere = np.zeros((n_comps, 17853))
    crp_cere = np.zeros((n_comps, 17853))
